...
require_once('header_proses.php');
 ...
 if ($kesiapan == 'Tidak') :
	...
	/*Jika Kesiapan Ya*/
		...
		$query2 = $conn->query("SELECT kesiapan FROM input_permohonan_kh_lab_parasit WHERE id = $id");
		...
		if ($old_kesiapan != $_POST['kesiapan']) :
			...
			$query2 = $conn->query("SELECT no_sampel FROM input_permohonan_kh_lab_parasit WHERE id = $newid");
			...
			if ($maxId > $id ) {
	 		...
	 		/*Jika Tidak Ada Id Yang lebih tinggi masuk sini*/
				...
				$objectDataParasit->edit("UPDATE input_permohonan_kh_lab_parasit SET no_sampel = '$awal_no_sampel' WHERE id ='$id'");
